...
void Set2dCurveData(uint r_gridsz, uint r_blcksz, double* r_x, double* r_y, 
  float2* r_plot_data, uint r_num, double2* r_last_data)
{
  Set2dCurveDataKernel<<<r_gridsz, r_blcksz>>>(r_x, r_y, r_plot_data, r_num);
  cudaMemcpy(&(r_last_data->x), r_x + r_num - 1, sizeof(double), 
    cudaMemcpyDeviceToHost);
  cudaMemcpy(&(r_last_data->y), r_y + r_num - 1, sizeof(double), 
    cudaMemcpyDeviceToHost);
}

void Set2dHistogramData(uint r_gridsz, uint r_blcksz, double* r_x, uint* r_y, 
  float2* r_plot_data, uint r_num)
{
  Set2dCurveSemiLogDataKernel<<<r_gridsz, r_blcksz>>>(r_x, r_y, r_plot_data, 
    r_num);
}
...
void SetPhaseSpaceData(uint r_gridsz, uint r_blcksz, double* r_x, 
                       ...
                       float2* r_plot_data, uint r_num)
{
  // Look for the first survived particle coordinates and use it as the default
  float2* loss_val;
  ...
  // Now if the particle is lost, its x and y is replaced with 0.0, 
  // just like the reference particle.
  Set2dPhaseSpaceDataKernel<<<r_gridsz, r_blcksz>>>(r_x, r_y, r_loss, 
    r_plot_data, r_tmp_x, r_tmp_y, r_num);
                ...
                uint r_num, uint* r_loss)
{
  ...
  cudaMemcpy(&(r_maxmin->x), r_partial_x + r_gridsz, sizeof(double), 
    cudaMemcpyDeviceToHost);
  cudaMemcpy(&(r_maxmin->y), r_partial_x + r_gridsz + 1, sizeof(double), 
    cudaMemcpyDeviceToHost);
  cudaMemcpy(&(r_maxmin->z), r_partial_y + r_gridsz, sizeof(double), 
    cudaMemcpyDeviceToHost);
  cudaMemcpy(&(r_maxmin->w), r_partial_y + r_gridsz + 1, sizeof(double), 
    cudaMemcpyDeviceToHost);
}
                  ...
                  double* r_partial_x, double4* r_maxmin, uint r_num)
{
  ...
  cudaMemcpy(&(r_maxmin->x), r_partial_x + r_gridsz, sizeof(double), 
    cudaMemcpyDeviceToHost); // xmax
  cudaMemcpy(&(r_maxmin->y), r_partial_x + r_gridsz + 1, sizeof(double), 
    cudaMemcpyDeviceToHost); // xmin
}
...
void UpdateHistogram2DKernelCall(uint* r_hist, double* r_input_x, 
  double* r_input_y, uint* r_input_loss, uint r_data_num, double4* r_maxmin, 
  uint r_bin_num_x, uint r_bin_num_y, uint r_thrd_num, uint r_blck_num)
{
  Histogram2DKernel<<<r_blck_num, r_thrd_num>>>(r_hist, r_input_x, r_input_y, 
    r_input_loss, r_data_num, r_maxmin->y, r_maxmin->x, r_maxmin->w, 
    r_maxmin->z, r_bin_num_x, r_bin_num_y);  
}

void SetHistogram2DCoordinateDataKernelCall(float2* r_data, double4* r_maxmin, 
  uint r_bin_num_x, uint r_bin_num_y, uint r_thrd_num, uint r_blck_num)
{
  SetHistogram2DCoordinateDataKernel<<<r_blck_num, r_thrd_num>>>(r_data, 
    r_maxmin->y, r_maxmin->x, r_maxmin->w, r_maxmin->z, r_bin_num_x, 
    r_bin_num_y);
}

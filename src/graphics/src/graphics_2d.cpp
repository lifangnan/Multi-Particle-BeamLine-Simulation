...
{
  Initialize(r_input_x, r_input_y, r_data_num, r_color, r_auto_update, 
    r_data_tracker_on);
}
                 ...
                 GLfloat* r_color, bool r_auto_update, bool r_data_tracker_on)
{
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(float2) * r_data_num, 0, 
    GL_DYNAMIC_DRAW);
  ...
  // TODO: replace 2 with sz of blocks+1
  cudaMalloc((void**)&partial_x2, sizeof(double) * (blck_num + 1) * 2); 
  cudaMalloc((void**)&partial_y2, sizeof(double) * (blck_num + 1) * 2);
}
...
void Curve2D::UpdateData()
{
  ...
  Set2dCurveData(blck_num, thrd_num, input_x, input_y, data, data_num, 
    &last_data);
  if(auto_update)
    FindMaxMin2D(blck_num, thrd_num, input_x, input_y, partial_x2, partial_y2, 
      maxmin, data_num);
...
/*------------- Histogram2D ----------------*/
Histogram2D::Histogram2D(double* r_input_x, double* r_input_y, 
  uint* r_input_loss, uint r_data_num, uint r_bin_num_x, uint r_bin_num_y)
  : Curve2D(r_input_x, r_input_y, r_bin_num_x * r_bin_num_y, NULL, true, false),
    input_loss(r_input_loss), bin_num_x(r_bin_num_x), bin_num_y(r_bin_num_y), 
    raw_data_num(r_data_num)
{
  InitHistogram2D();
  auto_update = true;
}
...
Histogram2D::Histogram2D(double* r_input_x, double* r_input_y, 
  uint* r_input_loss, uint r_data_num, double* r_avg_x, double* r_sig_x, 
  double* r_avg_y, double* r_sig_y, uint r_bin_num_x, uint r_bin_num_y)
  : Curve2D(r_input_x, r_input_y, r_bin_num_x * r_bin_num_y, NULL, false, false),
    input_loss(r_input_loss), bin_num_x(r_bin_num_x), bin_num_y(r_bin_num_y), 
    raw_data_num(r_data_num)
{
  InitHistogram2D();
...
void Histogram2D::InitHistogram2D()
{
  ...
  glBindBuffer(GL_ARRAY_BUFFER, 0);
  // register the vbo with cuda resource
  // cudaGraphicsGLRegisterBuffer(&cuda_color_vbo, color_vbo, 
  // 	cudaGraphicsMapFlagsNone);

  SetMaxMin(std::numeric_limits<double>::min(), 
    std::numeric_limits<double>::max(), std::numeric_limits<double>::min(), 
    std::numeric_limits<double>::max());

  thrd_num = 128;
  uint total_thread_num = std::pow(2, (int)std::ceil(std::log(raw_data_num) / 
    std::log(2)));
...
void Histogram2D::UpdateData()
{
  if (auto_update)
  {
    ...
    cudaMemcpy(x_h, input_x, sizeof(double) * raw_data_num, 
      cudaMemcpyDeviceToHost);
    double* y_h = new double[raw_data_num];
    cudaMemcpy(y_h, input_y, sizeof(double) * raw_data_num, 
      cudaMemcpyDeviceToHost);
    uint* loss_h = new uint[raw_data_num];
    cudaMemcpy(loss_h, input_loss, sizeof(uint) * raw_data_num, 
      cudaMemcpyDeviceToHost);
  ...
  else
  {
    ...
    SetMaxMin(avg_x_h - 6 * sig_x_h, avg_x_h + 6 * sig_x_h, 
      avg_y_h - 6 * sig_y_h, avg_y_h + 6 * sig_y_h);
  }

  cudaGraphicsMapResources(1, &cuda_vbo, 0); // count, resource, stream
  size_t num_bytes;
  cudaGraphicsResourceGetMappedPointer((void**)&data, &num_bytes, cuda_vbo);
  SetHistogram2DCoordinateDataKernelCall(data, maxmin, bin_num_x, bin_num_y, 
    thrd_num, blck_num); 
  cudaGraphicsUnmapResources(1, &cuda_vbo, 0);

  cudaMemset(hist, 0, sizeof(uint)*data_num);
  UpdateHistogram2DKernelCall(hist, input_x, input_y, input_loss, raw_data_num, 
    maxmin, bin_num_x, bin_num_y, thrd_num, blck_num);
...
void PhaseSpace::UpdateData()
{
  ...
  if(auto_update)
    FindMaxMin2D(blck_num, thrd_num, tmp_x, tmp_y, partial_x2, partial_y2, 
      maxmin, data_num);
  cudaGraphicsUnmapResources(1, &cuda_vbo, 0);
}
    ...
    min(r_min), input_loss(r_input_loss), hist_data_num(r_data_num)
{
  ...
  cudaMemcpy(input_x, h_input_x, sizeof(double) * r_bin_num, 
    cudaMemcpyHostToDevice);
  delete h_input_x;

  thrd_num = 128;
  uint total_thread_num = std::pow(2, (int)std::ceil(std::log(hist_data_num) / 
    std::log(2)));
  blck_num = total_thread_num / thrd_num;
...
void Histogram::UpdateData()
{
  ...
  size_t num_bytes;
  cudaGraphicsResourceGetMappedPointer((void**)&data, &num_bytes, cuda_vbo);
  UpdateHistogram(input_y, input_loss, hist, partial_hist, hist_data_num, 
    data_num, min, max, blck_num, thrd_num); 
  double width = (max - min) / data_num;
  double* h_input_x = new double[data_num];
  for(int i = 0; i < data_num; ++i)
    h_input_x[i] = min + width * (0.5 + i); 
  cudaMemcpy(input_x, h_input_x, sizeof(double)*data_num, 
    cudaMemcpyHostToDevice);
  delete h_input_x;

  // data_num == bin_num
  Set2dHistogramData(blck_num, thrd_num, input_x, hist, data, data_num); 
  cudaGraphicsUnmapResources(1, &cuda_vbo, 0);
}
...
#ifdef _DEBUG
    std::cout << "Curve " << i << ": xmax=" << vxmax[i] << ", xmin=" 
      << vxmin[i] << ", ymax=" << vymax[i] << ", ymin=" << vymin[i] 
      << std::endl;
#endif
  }
...
int Plot2D::InitPlot()
{
  ...
  if((vs = CreateShader(std::string(GetProjectTopDir() + 
    "/src/graphics/src/graphics_2d.v.glsl").c_str(), GL_VERTEX_SHADER)) == 0) 
    return 0;
  if((fs = CreateShader(std::string(GetProjectTopDir() + 
    "/src/graphics/src/graphics_2d.f.glsl").c_str(), GL_FRAGMENT_SHADER)) == 0) 
    return 0;
  ...
  if((nvs = CreateShader(std::string(GetProjectTopDir() + 
    "/src/graphics/src/graphics_2d_nonuniform_color.v.glsl").c_str(), 
    GL_VERTEX_SHADER)) == 0) return 0;
  if((nfs = CreateShader(std::string(GetProjectTopDir() + 
    "/src/graphics/src/graphics_2d_nonuniform_color.f.glsl").c_str(), 
    GL_FRAGMENT_SHADER)) == 0) return 0;
  ...
  attribute_coord2d_vc = glGetAttribLocation(program_varying_color, 
    attribute_name);
  ...
  uniform_transform_vc = glGetUniformLocation(program_varying_color, 
    uniform_name);
...
void Plot2D::PlotOneCurve(uint r_indx, uint r_nc)
{
  ...
  glm::mat4 transform = glm::translate(glm::scale(glm::mat4(1.0f), 
		glm::vec3(plots[r_indx]->scale_x, plots[r_indx]->scale_y, 1)), 
		glm::vec3(plots[r_indx]->offset_x, plots[r_indx]->offset_y, 0));
  glUniformMatrix4fv(uniform_transform_vc, 1, GL_FALSE, 
...
void Plot2D::DrawTicksGrids(uint r_indx)
{
  ...
  for(int i = 0; i < nticks; i++) 
  {
    ...
    ostr << std::setprecision(5)<< (i + low_i) * tickspacingy * 
      plots[r_indx]->label_scale_y;
                                    ...
                                    float r_width, float r_height)
{
  ...
  float offset_y = (2.0 * r_y + (r_height - window_height) + 2 * 
    w_height*r_indx) / window_height;
  float scale_x = r_width / window_width;
  float scale_y = r_height / window_height;

  return glm::scale(glm::translate(glm::mat4(1), 
    glm::vec3(offset_x, offset_y, 0)), glm::vec3(scale_x, scale_y, 1)); 
}
...
void Plot2D::DrawOneLastDataTracker(uint r_pindx, uint r_cindx, GLfloat* r_color)
{
  ...
  ostr_l << "(" << (plots[r_pindx]->curves)[r_cindx]->last_data.x * 
    plots[r_pindx]->label_scale_x << ", " << 
    (plots[r_pindx]->curves)[r_cindx]->last_data.y * 
    plots[r_pindx]->label_scale_y << ")";
...
{
  if((plots[r_indx]->xmarker).empty() && (plots[r_indx]->ymarker).empty()) 
    return;
  if(!(plots[r_indx]->xmarker).empty())
  {
    ...
    {
      // in real value coordinate
      left = -1.0 / plots[r_indx]->scale_x - plots[r_indx]->offset_x; 
  ...
  if(!(plots[r_indx]->ymarker).empty())
  {
    ...
    {
      std::cout << "put an Y marker at " << (plots[r_indx]->ymarker)[i] 
	<< std::endl;
      // in real value coordinate
      low = -1.0 / plots[r_indx]->scale_y - plots[r_indx]->offset_y; 

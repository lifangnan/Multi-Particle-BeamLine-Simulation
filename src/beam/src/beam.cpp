...
{
  // std::vector<double> temp_x = this->GetX();
  if (r_good_only){
    // std::vector<uint> loss_index = this->GetLoss();
    // double sum = 0;
    // int good_count = 0;
    ...
    // double mean = std::accumulate(std::begin(temp_x), std::end(temp_x), 0.0) / temp_x.size();
    // return mean;
...
double Beam::GetAvgY(bool r_good_only) const
{
  ...
  // std::vector<double> temp_y = this->GetY();
  // if (r_good_only){
  //   std::vector<uint> loss_index = this->GetLoss();
  //   double sum = 0;
  //   int good_count = 0;
  ...
  //   // return GetDataFromDevice(x_avg_good, 0);
  // }else{
  //   double mean = std::accumulate(std::begin(temp_y), std::end(temp_y), 0.0) / temp_y.size();
  //   return mean;
  //   // return GetDataFromDevice(x_avg, 0);
  // }
}

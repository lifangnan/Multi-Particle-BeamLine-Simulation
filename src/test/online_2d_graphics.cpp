...
void InitCUDA()
{
  // Must appear before cudaGraphicsGLRegisterBuffer and glut init
  cudaGLSetGLDevice(0); 
...
void Motion(int x, int y)
{
  ...
  if (button_states[wid] == 3) 
  {
    ...
    (windows[wid].plots[subplot_id[wid]])->scale_ratio += (trans[wid] - 
      (windows[wid].plots[subplot_id[wid]])->scale_ratio) * inertia;
  }
  else if (button_states[wid] == 1)
  {
    ...
    if ((windows[wid].plots[subplot_id[wid]])->offset_x > 0)
      (windows[wid].plots[subplot_id[wid]])->offset_mouse_x += 
	dx / width * (windows[wid].plots[subplot_id[wid]])->offset_x * 10;
    else
      (windows[wid].plots[subplot_id[wid]])->offset_mouse_x -= dx / width * 
	(windows[wid].plots[subplot_id[wid]])->offset_x * 10;
    if ((windows[wid].plots[subplot_id[wid]])->offset_y < 0)
      (windows[wid].plots[subplot_id[wid]])->offset_mouse_y += dy / height * 
	(windows[wid].plots[subplot_id[wid]])->offset_y;
    else
      (windows[wid].plots[subplot_id[wid]])->offset_mouse_y -= dy / height * 
	(windows[wid].plots[subplot_id[wid]])->offset_y;
  }
...
void Key(unsigned char key, int /*x*/, int /*y*/)
{
  switch (key)
  {
    ...
    case 's':
    {
      ...
      std::cout << "Input x&y range for the " << windows[wid].plots.size() - 
	subplot_id[wid] << " plot: ";
...
int main(int argc, char* argv[])
{
  ...
  glewInit();
  windows[0].SetWindowSize(glutGet(GLUT_WINDOW_WIDTH), 
  ...
  Histogram2D* h2d1 = new Histogram2D(beam.x, beam.xp, beam.loss, 
    beam.num_particle, 120, 75);
  curves.push_back(h2d1);
  Subplot2D subplot01;
  subplot01.AddCurve(h2d1);
  subplot01.UpdateSubplot();

  Histogram2D* h2d2 = new Histogram2D(beam.y, beam.yp, beam.loss, 
    beam.num_particle, 120, 75);
  ...
  Histogram2D* h2d3 = new Histogram2D(beam.phi_r, beam.w, beam.loss,
    beam.num_particle, beam.phi_avg_r, beam.phi_sig_good, beam.w_avg_good, 
    beam.w_sig_good, 120, 75);
  ...
  glewInit();
  windows[1].SetWindowSize(glutGet(GLUT_WINDOW_WIDTH), 
    glutGet(GLUT_WINDOW_HEIGHT));
  windows[1].SetBorderWidth(10 * 7, 10 * 2);
  windows[1].SetTickSize(10);

  Subplot2D subplot11, subplot12, subplot13, subplot14;
  Histogram* hist1 = new Histogram(beam.x, beam.loss, beam.num_particle, 
    -0.025, 0.025, red);
  hist1->SetLabelY("x profile");
  Histogram* hist2 = new Histogram(beam.y, beam.loss, beam.num_particle, 
    -0.025, 0.025, green);
  ...
  Histogram* hist3 = new Histogram(beam.phi_r, beam.loss, beam.num_particle, 
    -M_PI*0.5, M_PI*0.5, yellow);
  ...
  Histogram* hist4 = new Histogram(beam.w, beam.loss, beam.num_particle, 
    5, 810, blue);
  ...
  glewInit();
  windows[2].SetWindowSize(glutGet(GLUT_WINDOW_WIDTH), 
    glutGet(GLUT_WINDOW_HEIGHT));
  ...
  cudaMemcpy(xaxis.d_ptr, h_axis, sizeof(double) * monitor_num, 
    cudaMemcpyHostToDevice);
  delete [] h_axis;

  Curve2D* cxsig = new Curve2D(xaxis.d_ptr, gldata.xsig.d_ptr, monitor_num, red);
  cxsig->SetLabelY("x sigma");
  Curve2D* cysig = new Curve2D(xaxis.d_ptr, gldata.ysig.d_ptr, 
      monitor_num, green);
  ...
  Curve2D* cphisig = new Curve2D(xaxis.d_ptr, gldata.phisig.d_ptr, 
    monitor_num, yellow);
  ...
  Curve2D* cwsig = new Curve2D(xaxis.d_ptr, gldata.wsig.d_ptr, 
    monitor_num, blue);
  ...
  Curve2D* cemitx = new Curve2D(xaxis.d_ptr, gldata.xsig.d_ptr, 
    monitor_num, red);
  cemitx->SetLabelY("x emittance");
  Curve2D* cemity = new Curve2D(xaxis.d_ptr, gldata.yemit.d_ptr, 
    monitor_num, green);
  ...
  Curve2D* closs_loc = new Curve2D(xaxis.d_ptr, gldata.loss_local.d_ptr, 
    monitor_num, orange);
  ...
  Curve2D* closs = new Curve2D(xaxis.d_ptr, gldata.loss_ratio.d_ptr, 
    monitor_num, magenta);
